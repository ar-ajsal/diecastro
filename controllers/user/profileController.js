...
const userProfile = async (req, res) => {
    try {
        ...
        res.render("profile", {
            user: userData, addresses: addressData,orders:limitedorders,totalOrders,totalSpending,session: req.session,
        })
...
const changeEmail = async (req, res) => {
    try {
        res.render('change-email',{ session: req.session,})
    } catch (error) {
        console.error('Error:', error)
        res.redirect("/pageNotFound")
    }
...
const changePassword = async (req, res) => {
    try {
        res.render('changepass-email-valid.ejs')
    } catch (error) {
        console.error('Error:', error)
        res.redirect("/pageNotFound")
    }
...
const changePassEmailValid = async (req, res) => {
    try {
        ...
        if (!userData) {
            return res.render("changepass-email-valid", {
                message: "User not found. Please log in again.",
            });
        }
