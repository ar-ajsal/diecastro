...
const validateName = (name) => {
  ...
  if (!trimmedName) {
    ...
    if (!/^[a-zA-Z0-9\s\-_]+$/.test(trimmedName)) {
      errors.push(
        "Category name can only contain letters, numbers, spaces, hyphens, or underscores"
      );
    }
  }
...
const validateDescription = (description) => {
  const errors = [];
  const trimmedDescription = sanitizeInput(description);
  ...
  return { trimmedDescription, errors };
};
...
const addCategory = async (req, res) => {
  try {
    ...
    // Validate name
    const { trimmedName, errors: nameErrors } = validateName(name);

    // Validate description
    const { trimmedDescription, errors: descErrors } = validateDescription(description);
...
const editCategory = async (req, res) => {
  try {
    ...
    if (existingCategory) {
      console.log("Category name already in use:", trimmedName);
      return res
        .status(400)
        .json({ success: false, message: "Category with this name already exists" });
    }
